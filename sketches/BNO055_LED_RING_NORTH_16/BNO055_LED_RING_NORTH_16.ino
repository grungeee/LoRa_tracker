...
void onEspNowReceive(const uint8_t* mac, const uint8_t* data, int len) {
  ...
  TARGETS[0].lat = packet.lat;
  TARGETS[0].lng = packet.lon;
  greenLastRxMs = millis();
...
void loop() {
  ...
  if (now - lastPrintMs >= SERIAL_PRINT_INTERVAL_MS) {
    ...
    Serial.print(espNowReady ? (greenLastRxMs > 0 ? F("OK") : F("WAIT")) : F("FAIL"));
    Serial.print(F(" brightness="));
    Serial.print(brightnessBoosted ? F("100") : F("10"));
    Serial.print(F(" pkts="));
    Serial.print(espNowRxCount);
    Serial.print(F(" bad="));
    Serial.print(espNowBadLenCount + espNowBadMagicCount + espNowInvalidGpsCount);
    ...
    Serial.print(greenLastRxMs > 0 && millis() - greenLastRxMs < GREEN_RX_FRESH_MS ? F("fresh") : F("none"));
    Serial.print(F(" rlat="));
